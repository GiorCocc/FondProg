...
private:
	...
	//metodo per l'inserimento degli elementi di un albero all'interno di una lista
	void insertListR(link &h, List<Item> &l)
	{
		if (h == 0) //se l'albero è vuoto non ho nulla da inserire
			return;

		insertOrdered(l, h->item); //inserimento ordinato all'interno della lista

		insertListR(h->l, l); //ricorsione sul sottoalbero sinistro
		insertListR(h->r, l); //ricorsione sul sottoalbero destro
	}
...
public:
	...
	void merge(BST<Item, Key> &b, LList<Item> &l)
	{
		insertListR(head, l);	//inserimento nella lista rispetto al primo albero
		insertListR(b.head, l); //inserimento nella lista rispetto al secondo albero
	}
