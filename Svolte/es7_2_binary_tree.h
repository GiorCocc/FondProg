...
private:
    ...
    {
        if (theRootA == NULL) //controllo che il primo albero sia vuoto
        {
            if (theRootB != NULL) //controllo che il secondo albero non sia vuoto
                return 0;         //hanno struttura diversa perchè il primo è vuoto e il secondo no
        }
        else
        {
            if (theRootB == NULL) //controllo che il secondo sia vuoto
                return 0;         //non avendo struttura non sono confrontabili

            int ris;
            ris = equal_structure(theRootA->lChildptr, theRootB->lChildptr); //chiamo ricorsivamente la funzione nel sottoalbero sinistro dei due alberi
            if (ris == 0)                                                    //controllo che il risultato della ricorsione nel sottoalbero sinistro sia 0
                return 0;                                                    //gli alberi sono diversi

            ris = equal_structure(theRootA->rChildptr, theRootB->rChildptr); //chiamo ricorsivamente la funzione nel sottoalbero destro dei due alberi
            if (ris == 0)                                                    //controllo che il risultato della ricorsione nel sottoalbero sinistro sia 0
                return 0;                                                    //gli alberi sono diversi
        }
        return 1; //gli alberi hanno la stessa struttura (stesso numero di nodi, stesso numero di livelli, stesso numero di figli per sottoalbero)
    }
